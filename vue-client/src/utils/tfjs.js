...
export function processDetectionResults(res, detectionThreshold) {
  ...
  function process_pred(res) {
    ...
    let max_score_index = cls_detections.reduce((imax, x, i, arr) => (x > arr[imax] ? i : imax), 0)
...
export async function nmsDetectionProcess(boxes, scores, thresholdIou) {
  ...
  try {
    _nmsDetections = await tf.image.nonMaxSuppressionAsync(boxes, scores, 100, thresholdIou)
