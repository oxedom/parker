...
const ClientRender = ({
  ...
  const handleAutoDetect = (predictionsArr) => 
  {

        let action = {
          event: "addManyRois",
          payload: { predictionsArr },
        };
  ...
  const autoDetectFrame = async (model) => 
  {
    if (
      typeof webcamRef.current !== "undefined" &&
      webcamRef.current !== null &&
      ...
      if (boxes.length > 0) {
        ...
        for (let i = 0; i < detectionIndices.length; i++) {
          ...
          let width = x2 - x1;
          let height = y2 - y1;
          roiObj.cords.bottom_y = Math.round(y2);
          roiObj.cords.left_x = Math.round(x2);
          roiObj.cords.top_y = Math.round(y1);
          roiObj.cords.right_x = Math.round(x1);
          roiObj.cords.width = width;
        ...
        handleAutoDetect(predictionsArr)
  ...
  {
    console.log("Auto detect HAS RUN ");
