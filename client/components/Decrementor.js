...
const Decrementor  = ({min, max, step, setter, value, label }) => {
 ...
 function handleDecrement(e) 
    ...
    return ( <div className="flex flex-col pt-1 items-center">
        <label className="
        border m-1
        "> {label} </label>
        <div className="grid grid-cols-3 items-center justify-center justify-items-center">

        <button value={value}  label="decrease"
        onMouseUp={() => { clearInterval(id_down)}}
        onMouseDown={(e) => {
            
            setId_down(setInterval(() => {
handleDecrement(e)
        }, 50)) }}   className={`${btnClass} bg-gray-300`}>-</button> 

         <span className=" text-lg border-gray-200 text-center "> {value}</span>
        <button value={value} label="increase" 
        onMouseUp={() => { clearInterval(id_up)}}
        onMouseDown={(e) => {
            
           setId_up(setInterval(() => {
handleIncrement(e)
        }, 50)) }}
        
        
        className={`${btnClass} bg-gray-400  `}>+</button> 
